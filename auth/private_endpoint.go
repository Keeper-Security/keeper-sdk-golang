...
func (endpoint *keeperEndpoint) ConfigurationStorage() IConfigurationStorage {
...
func (endpoint *keeperEndpoint) CommunicateKeeper(path string, request []byte, sessionToken []byte) (response []byte, err error) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		if rs.StatusCode == 200 && rs.Header.Get("Content-Type") == "application/octet-stream" {
			...
			if body, err = io.ReadAll(rs.Body); err == nil {
				var apiError api.KeeperApiErrorResponse
...
func (endpoint *keeperEndpoint) ConnectToPushServer(wssRequest *proto_auth.WssConnectionRequest) (result IPushEndpoint, err error) {
	var payload []byte
	...
	result = NewWebSocketEndpoint(pushUrl.String(), transmissionKey)
	return
}
